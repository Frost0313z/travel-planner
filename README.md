...
**"3월 15일에 어디로 여행갈까?"** 라는 질문에 AI와 실제 지도 데이터로 답해주는 CLI 프로그램입니다.
날짜 하나만 입력하면 추천 여행지 → 날씨/행사 → 실제 맛집 → 1일 일정까지 담은 리포트를 자동으로 만들어줍니다.
...
> **목차**
>
> - [💡 이 프로그램은 무엇을, 어떻게 하나요?](#-이-프로그램은-무엇을-어떻게-하나요)
> - [🚀 빠른 시작](#-빠른-시작)
> - [🖥️ 실행 예시](#️-실행-예시)
> - [📋 기능](#-기능)
...
> - [🧠 설계 노트 (더 깊이 알고 싶다면)](#-설계-노트-더-깊이-알고-싶다면) — 파이프라인 구조, 캐싱, 도시명 정규화, 오류 이력, 알려진 한계, 키 관리 등
> - [📄 라이선스](#-라이선스)
...
사람이 직접 검색하지 않고, AI와 실제 지도 데이터를 조합해서 여행 계획을 자동으로 만들어줍니다. 동작은 딱 세 단계입니다.

```
"3월 15일에 여행 가고 싶어"
        │
        ▼
1️⃣ [LLM] 언제·어디로 가면 좋을지 추천    ──▶  "제주, 유채꽃 시즌이라 날씨 좋고..."
        │  (도시명·날씨·행사·이유를 JSON으로)
        ▼
2️⃣ [지도 API] 그 도시의 실제 맛집 검색   ──▶  진짜 존재하는 맛집 5곳 (이름·주소·링크)
        │  (NAVER API HUB 지역 검색)
        ▼
3️⃣ [LLM] 위 정보를 하나의 리포트로 정리  ──▶  results/2026-03-15_travel_plan.md
```

<br>

포인트는 **1번(AI 추천)이 2번(지도 검색)의 입력으로 그대로 이어진다는 것**입니다. AI가 "제주"를 추천하면, 그 "제주"라는 글자를 그대로 다음 API 호출에 사용합니다. 그래서 1번의 출력 형식을 엄격한 JSON으로 강제하고, 2번에서는 도시명 표기가 들쭉날쭉해도("제주"/"제주도"/"제주특별자치도") 문제없이 검색되도록 정규화 단계를 넣었습니다.

세 단계 중 어느 하나가 실패해도(지도 API 키가 없거나 인증 오류 등) 프로그램은 멈추지 않고 "데이터 없음"으로 표시한 뒤 나머지는 정상적으로 완성합니다 — 왜 이렇게 설계했는지는 [설계 노트](#-설계-노트-더-깊이-알고-싶다면)에서 이유까지 다룹니다.
...
cp .env.example .env   # 이후 .env에 실제 키 값 입력 (아래 API 키 설정 방법 참고)
...
| 2 | 🌤️ 1차 추천 생성 (LLM) | 입력 날짜를 바탕으로 추천 도시/날씨/행사/추천 이유를 JSON으로 생성, 스키마 불일치 시 1회 재시도 |
| 3 | 🏷️ 도시명 정규화 | "제주도"/"제주특별자치도" 같은 표기를 검색 전에 표준 표기("제주")로 정규화 |
| 4 | 🍽️ 맛집 검색 (지도/장소 API) | NAVER API HUB 지역 검색으로 맛집 최대 5곳 조회, 0건이면 검색어를 넓혀 1회 재검색 |
| 5 | 📝 최종 리포트 생성 (LLM) | 1차 추천 + 맛집 목록 + 오류 목록을 종합해 Markdown 리포트 생성 |
| 6 | 💾 결과 저장 | `results/`에 원본 데이터 JSON과 최종 리포트 Markdown 저장 |
| 7 | 📜 오류 이력 누적 | 실행마다 발생한 오류를 `results/errors_log.jsonl`에 한 줄씩 누적, 여러 실행 이력을 한 파일로 조회 가능 |
| 8 | 🛡️ 오류 처리 | API 키 미설정 시 즉시 종료, 지도 API 실패 시 "데이터 없음"으로 계속 진행 |
| 9 | ⚡ 결과 캐싱 (보너스) | 같은 `--date`로 재실행 시 저장된 결과가 있으면 API 호출 없이 그대로 재사용 |
...
- `results/errors_log.jsonl`: 실행마다 `{run_date, executed_at, errors}` 한 줄씩 누적되는 오류 이력 (아래 [실행 간 오류 이력](#-실행-간-오류-이력) 참고).
...
├── results/                   실행 결과 저장 위치 (errors_log.jsonl 포함)
└── travel_planner/
    ├── __init__.py
    ├── cli.py                 argparse 인자 파싱 + 날짜 형식 검증
    ├── config.py               .env 로드 + API 키 누락 시 안내 후 종료
    ├── data.py                 상수, LLM 프롬프트, API 엔드포인트, 도시명 별칭 표
    ├── llm_client.py           OpenAI 연동 (1차 추천, 최종 리포트 생성, JSON 파싱 재시도)
    ├── place_search.py        NAVER API HUB 지역 검색 연동 (도시명 정규화, 맛집 검색, 재검색)
    ├── storage.py              결과 JSON/Markdown 저장, 오류 이력 로그, 캐시 조회
...
```
* f7b13d8 docs: README를 '무엇을/어떻게 하는지' 먼저 설명하는 흐름으로 재구성
* 62aab30 chore: 예시 결과물을 최신 코드로 재생성 (errors_log.jsonl 포함)
* 1c90faa feat: 실행 간 오류 이력 누적 로그 구현 (네이토 사전평가 #9 반영)
* c4e0ee8 fix: 도시명 정규화가 2글자 지명을 1글자로 잘못 줄이는 버그 수정 + 맛집 0건 시 카테고리 확장 재검색 추가 (네이토 사전평가 #17 반영)
* 38bf9fc docs: 도시명 정규화 구현 내용을 기능표/설계노트에 반영, 개발환경 증빙 갱신
...
위 [💡 이 프로그램은 무엇을, 어떻게 하나요?](#-이-프로그램은-무엇을-어떻게-하나요)에서 "무엇을 하는지"를 봤다면, 여기서는 "왜 이렇게 만들었는지"를 다룹니다.
...
LLM이 반환하는 `recommended_city`는 "제주"/"제주도"/"제주특별자치도"처럼 표기가 일정하지 않다. `place_search.normalize_city()`로 검색 전에 표준 표기 하나로 정규화한다.

1. **별칭 표 우선 조회** — `제주도`, `서울특별시`, `부산광역시`처럼 자주 나오는 표기를 표준형으로 매핑한 표(`CITY_ALIASES`)를 먼저 찾는다.
2. **접미사 제거로 대체 처리** — 표에 없는 이름은 `특별자치도`/`광역시`/`특별시`/`시`/`도` 같은 행정구역 접미사를 순서대로 떼어낸다 (예: `전라남도` → `전라남`). 단, 떼어낸 결과가 1글자면 지명이 아닐 가능성이 높아(`독도` → `도`를 떼면 `독`) 원본을 그대로 둔다 — 실제로 이 케이스를 테스트하다 발견해서 가드를 추가했다.
3. **검색 자체도 한 번 더 넓힌다** — 정규화된 도시명으로 "{도시} 맛집" 검색이 0건이면, "{도시} 음식점"으로 카테고리를 넓혀 1회 재검색한다.
4. **정규화·재검색이 실제로 일어나면 로그로 남긴다** — `- 도시명 정규화: "제주특별자치도" -> "제주"`, `- "OO 맛집" 0건, "OO 음식점"로 재검색합니다...`처럼 출력해, 실행 로그만 보고도 어떤 변환이 있었는지 알 수 있다.

세부 지역 추출(예: "강남"에서 "서울 강남구" 유추)이나 LLM에 재질문하는 단계까지는 포함하지 않았다 — 표기 정규화 + 카테고리 재검색만으로 이번 미션이 요구하는 "정규화 전략"은 충족되며, 세부 지역 추출은 별도 지오코딩 API가 필요한 범위로 판단했다.

<br>

### 📜 실행 간 오류 이력

각 실행의 `errors`는 그 실행의 결과 JSON(`results/{date}_travel_plan.json`)에만 남기 때문에, 실행을 여러 번 했을 때 "언제 어떤 오류가 몇 번 있었는지"를 한눈에 보기 어렵다는 문제가 있었다.

그래서 `storage.append_error_log()`가 실행마다 `{"run_date", "executed_at", "errors"}` 한 줄을 `results/errors_log.jsonl`에 append한다. 오류가 없는 실행도 빈 `errors: []`로 기록되므로, 이 파일 하나만 보면 지금까지의 모든 실행 이력을 시간순으로 확인할 수 있다.

```bash
# 지금까지 오류가 있었던 실행만 보기
cat results/errors_log.jsonl | python -c "import sys,json; [print(l) for l in sys.stdin if json.loads(l)['errors']]"
```
...
| 한계 | 현재 동작 | 이유 |
|---|---|---|
| 세부 지역 추출 미지원 | 도시명 표준화 + 카테고리 확장 재검색까지만 수행 | "강남"처럼 여러 지역에 걸친 이름의 세부 지역 유추는 지오코딩 API가 필요한 범위 밖 작업 |
| 오류 로그에 자동 알림 없음 | `errors_log.jsonl`은 파일로만 쌓이고 별도 알림은 없음 | 반복 실행 환경(cron 등)에서 쓰려면 로그를 스캔해 알림을 보내는 래퍼가 별도로 필요 |
